...
function displayRecords(){   
    ...
    zsi.json.loadGrid({
         table  : "#grid"
        ,url   : controller_url +  "get_json"
        ,td_body: [ 
            function(d){
                return     bs({name:"denomination_ref_id[]",type:"hidden",value: d.denomination})
                        +  bs({name:"cb[]",type:"checkbox"});
            }
            ,function(d){ return bs({name:"denomination[]",value: d.denomination}); }
...
function displayBlankRows(){       
    ...
    zsi.json.loadGrid({
        ...
        ,td_body: [ 
            function(d){
                return     bs({name:"denomination_ref_id[]",type:"hidden"})
                        +  bs({name:"cb[]",type:"checkbox"});
            }            
            ,function(d){ return bs({name:"denomination[]"}); }
        ]
    });
