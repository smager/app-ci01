...
CREATE DEFINER=`zsi`@`%` FUNCTION `getExpenseSum2`(p_store_loc_id int(5), p_date date) RETURNS decimal(7,2)
    DETERMINISTIC
BEGIN
 DECLARE lvl decimal(7,2);
    select SUM(exp_amt) INTO lvl from store_loc_exp_dtls_v where store_loc_id = p_store_loc_id
...
CREATE DEFINER=`zsi`@`%` FUNCTION `getUnitDesc`(p_unit_id int) RETURNS varchar(100) CHARSET latin1
    DETERMINISTIC
BEGIN
    DECLARE lvl varchar(100);
    SELECT unit_desc INTO lvl FROM units WHERE unit_id=p_unit_id;
